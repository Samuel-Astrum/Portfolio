...
public class GraphAlgorithms {
    ...
    public static <T> List<Vertex<T>> bfs(Vertex<T> start, Graph<T> graph) {
        if (start == null || graph == null || !graph.getVertices().contains(start)) {
            throw new IllegalArgumentException("inputs cannot be null and start must be in graph");
        }
        ...
        q.add(start);
        while (!q.isEmpty()) {
            Vertex<T> key = q.remove();
            List<VertexDistance<T>> adj = graph.getAdjList().get(key);
     ...
     * @param list ordered list of traversal
     */
    private static <T> void dfsHelper(Vertex<T> start, Graph<T> graph, List<Vertex<T>> list) {
        list.add(start);
        List<VertexDistance<T>> adj = graph.getAdjList().get(start);
        for (VertexDistance<T> x : adj) {
            if (!list.contains(x.getVertex())) {
                dfsHelper(x.getVertex(), graph, list);
            }
        }
    }
    ...
    public static <T> Map<Vertex<T>, Integer> dijkstras(Vertex<T> start,
                                                        Graph<T> graph) {
        List<Vertex<T>> visitedSet = new ArrayList<Vertex<T>>();
        Map<Vertex<T>, VertexDistance<T>> distanceMap = new HashMap<Vertex<T>, VertexDistance<T>>();
        Queue<Vertex<T>> priority = new PriorityQueue<Vertex<T>>();
